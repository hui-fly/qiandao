    ...
    bulidCal: function(iYear, iMonth, signsList) {
        ...
        for (var i = 0; i < 7; i++) {
            for (var j = 0; j < 7; j++) {
                ...
                if ("1"===signsList[i*7+j]) {//signsList由后端返回，key值是[0,1,0,1...]的形式
...
app.controller("myCtrl", function($scope) {
    ...
    $scope.qiandao = function() {
        ...
        var current = new Date(year,month-1,1); //返回本月1号
		//确定今天日期在表格中的位置
		var iDayOfFirst = current.getDay(); //先获取本月第一天（5月1号）是星期几
		var itoday = day + iDayOfFirst - 1 + 7; //然后今天的日期所对应的下标就是itoday
		signsList[itoday] = "1";              //对应的元素变1
        
        //发送数据
        // param.data = {"today" : itoday}
        var isSuccess = _app.request(param);
        // if (isSuccess){
            // signsList = $('.data').val();   //获取input的value值
    ...
    $scope.date       = calUtil.bulidCal(calUtil.showYear, calUtil.showMonth, signsList); //根据当前年月日和signsList生成日历
	//日历栏上的年月
